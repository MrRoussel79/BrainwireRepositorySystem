...
def Home():
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Candidate Screening Test", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
    ...
    filemenu3.add_command(label="Take Test", command=mainTest)
...
def mainTest():
    result = tkMessageBox.askquestion('Candidate Screening Test', 'TEST', icon="warning")
def AddNewForm():
    ...
    lbl_productname = Label(MidAddNew, text="Candidate Name:", font=('arial', 25), bd=10)
...
def ViewForm():
    ...
    lbl_text = Label(TopViewForm, text="View Candidates", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Candidate Screening Test', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Candidate Screening Test/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('Candidate Screening Test', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
